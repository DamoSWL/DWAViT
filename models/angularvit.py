    ...
    def forward_feature(self,x):
        ...
        if self.attn_type == 'exp':
            ...
            if self.attn_type == 'cos':
                ...
                if self.simplified:
                    theta =(q @ k.transpose(-2, -1))
                    theta = -(math.pi/2-0.4)*theta + math.pi/2
                else:
...
class AngularLayer(nn.Module):
    def __init__(self,embed_dim=64,
        ...
        self.attention = AngularAttention(embed_dim=embed_dim,num_heads=self.num_heads,
                                        ...
                                        temperature=temperature,simplified=simplified)
...
@register_model
def angular_tiny_exp_224(pretrained=False, temperature=0.1, **kwargs):
    model = AngularViT(embed_dim=[64,128,256,512], depth=[2,4,18,2], num_heads=[1,2,4,8], 
                    n_window=[(8,7),(4,3),(2,1),(1,1)],
                    # n_window=[(10,7),(7,4),(4,1),(1,1)],
                    kv_pooling=[0,0,0,0],  mlp_ratio=4., attn_type='exp', temperature=temperature,  **kwargs)
    return model


@register_model
def angular_tiny_quad_224(pretrained=False,temperature=0.1,simplified=False, **kwargs):
    model = AngularViT(embed_dim=[64,128,256,512], depth=[2,4,18,2], num_heads=[1,2,4,8], 
                    n_window=[(8,7),(4,3),(2,1),(1,1)],
                    # n_window=[(10,7),(7,4),(4,1),(1,1)],
                    kv_pooling=[0,0,0,0],  mlp_ratio=4., attn_type='quad',temperature=temperature,simplified=simplified, **kwargs)
    return model


@register_model
def angular_small_quad_224(pretrained=False,temperature=0.1,simplified=False, **kwargs):
...
@register_model
def angular_base_quad_224(pretrained=False,temperature=0.25,simplified=False, **kwargs):
    model = AngularViT(embed_dim=[96,192,384,768], depth=[4,8,24,4], num_heads=[1,2,4,8], 
                    n_window=[(8,7),(4,3),(2,1),(1,1)],
                    # n_window=[(10,7),(7,4),(4,1),(1,1)],
                    kv_pooling=[0,0,0,0],  mlp_ratio=4., attn_type='quad',temperature=temperature,simplified=simplified, **kwargs)
    return model
...
@register_model
def angular_tiny_cos_224(pretrained=False,temperature=0.1, **kwargs):
    model = AngularViT(embed_dim=[64,128,256,512], depth=[2,4,18,2], num_heads=[1,2,4,8], 
                    n_window=[(8,7),(4,3),(2,1),(1,1)],
                    # n_window=[(10,7),(7,4),(4,1),(1,1)],
                    kv_pooling=[0,0,0,0],  mlp_ratio=4., attn_type='cos',temperature=temperature, **kwargs)
    return model


@register_model
def angular_small_cos_224(pretrained=False,temperature=0.1, **kwargs):
    model = AngularViT(embed_dim=[80,160,320,640], depth=[3,6,21,3], num_heads=[1,2,4,8], 
                    n_window=[(8,7),(4,3),(2,1),(1,1)],
                    # n_window=[(10,7),(7,4),(4,1),(1,1)],
                    kv_pooling=[0,0,0,0],  mlp_ratio=4., attn_type='cos',temperature=temperature,  **kwargs)
...
@register_model
def angular_base_cos_224(pretrained=False,temperature=0.25, **kwargs):
    model = AngularViT(embed_dim=[96,192,384,768], depth=[4,8,24,4], num_heads=[1,2,4,8], 
                    n_window=[(8,7),(4,3),(2,1),(1,1)],
                    # n_window=[(10,7),(7,4),(4,1),(1,1)],
                    kv_pooling=[0,0,0,0],  mlp_ratio=4., attn_type='cos',temperature=temperature,  **kwargs)
...
if __name__=='__main__':
    ...
    model = create_model('angular_base_quad_224')
